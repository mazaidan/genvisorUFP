...
suptitle('PND formation from CPC, PTrak and AeroTrak')
...
figure(2); fig = gcf;
fig.Position = [120 120 690 690].*1.5;
suptitle('PNSD calculation from PND')
for n= 1:8
    subplot(4,2,n);
...
for n= 1:8
    ...
    figure(3); fig = gcf;
    fig.Position = [120 120 690 690].*1.5;
    suptitle('PMD calculation from PND')
    subplot(4,2,n);
    ...
    plot(x,y,'r');hold off
    xlabel('PMD [Hussein]');ylabel('PMD1 [Zaidan]');
    title(['Bin ',num2str(n), ' : ',num2str(Dp_min(1,n)*1e6),' - ',num2str(Dp_max(1,n)*1e6),' $\mu$m'], 'interpreter','latex')
    set(findall(fig,'-property','FontSize'),'FontSize',22);
    
    figure(4); fig = gcf;
    fig.Position = [120 120 690 690].*1.5;
    suptitle('PMSD calculation from PMD')
    subplot(4,2,n);
    ...
    plot(x,y,'r');hold off
    xlabel('PMSD [Hussein]');ylabel('PMSD1 [Zaidan]');
    title(['Bin ',num2str(n), ' : ',num2str(Dp_min(1,n)*1e6),' - ',num2str(Dp_max(1,n)*1e6),' $\mu$m'], 'interpreter','latex')
    set(findall(fig,'-property','FontSize'),'FontSize',22);
...
title('Calculating PM$_{2.5}$ from PMSD','interpreter','latex');
hold on;
x = linspace(0,max(PM25));
y = linspace(0,max(PM25));
plot(x,y,'r');hold off
xlabel('PM$_{2.5}$ [Hussein]','interpreter','latex');
ylabel('PM$_{2.5}$ [Zaidan]','interpreter','latex');
set(findall(fig,'-property','FontSize'),'FontSize',22);


%% Scatter plot between PM2.5
% LCS vs DustTrak, LCS vs SidePak, LCS vs CPC + Ptrak + AT
% In conclusion, actually, LCSs are almost the same quality as CPC+Ptrak+AT
% But, in this case, we calibrate LCS based on DustTrak database
...
grid on
scatter(X,Y2,'r');scatter(X,Y3,'g');
set(gca, 'XScale', 'log')
set(gca, 'YScale', 'log')
xlabel('Low-Cost Sensors','interpreter','latex')
ylabel('Reference Instruments','interpreter','latex')
xlim([MINx MAXx]);ylim([MINy MAXy]);
x = linspace(MINx,MAXx,1000);
y = linspace(MINy,MAXy,1000);
plot(x,y,'r');
legend('SP','DT','CPC-PTrak-AT','ref.line','location','SouthEast','interpreter','latex')
...
% PNSD
T1=datetime(PNSD(2:end,1:6));
PNSD_DoY = PNSD(2:end,7);
PNSD_c   = PNSD(2:end,8:end);
PNSD1 = timetable(T1,PNSD_c);
PNSD2 = sortrows(PNSD1);
PNSD3 = PNSD2(1:end-1,:);
%PND2 = unique(PND1);
%clear T1 PND_DoY PND_c
...
% PMSD
T1=datetime(PMSD(2:end,1:6));
PMSD_DoY = PMSD(2:end,7);
PMSD_c   = PMSD(2:end,8:end);
PMSD1 = timetable(T1,PMSD_c);
...
% DATAs = DATAsmoking
    ...
    DATAs4 =[AeroTrak_met(:,1:6),AT_T,AT_RH,CO_T,CO_RH,CO_P, ...
        DustTrak_c,SidePak_c,PND_c,PMD_c,PNSD_c,PMSD_c, ...
...
clearvars -except DATA1 DATA2 DATAs2 DATAk2 DATAn2 DATA3 DATA4 DATA4_label %DATAs DATAk DATAn syn_s syn_k syn_n
 
%% ANALYZE MEASUEEMENT DATA QUALITY and INTERPOLATION when merging the data
...
% From the above analysis,it seems DATA2 is the most reliable interpolation
...
%% AFTER SYNCRONIZATION (DATA2) and using PNSD
figure(5);fig=gcf; 
fig.Position = [100 100 540 400].*2.5;
FS=26; 
lc =1e0; hc=10e5;
...
%plot(DATA.T1(Exp_smoking,1),DATA.DustTrak_c(Exp_smoking,2),'b.');
hold off
%legend('PMD','$\mathcal{L}_{1}$','$\mathcal{L}_{2a}$','$\mathcal{L}_{2b}$','interpreter','latex')
legend('PM$_{2.5}$ [LCSs]','interpreter','latex')
ylim([1e-1 1e3]); grid on
xlabel('Time Index','interpreter','latex')
ylabel('PM$_{2.5}$ ($\mu$g/m$^3$)','interpreter','latex')
title('PM$_{2.5}$ Smoking','interpreter','latex')
...
nexttile % subplot(222)
plot(DATA.T1(Exp_gas,1),DATA.LCS_G1(Exp_gas,1),'b.');
%plot(DATA.T1(Exp_gas,1),DATA.DustTrak_c(Exp_gas,2),'b.');
hold off
legend('PM$_{2.5}$ [LCSs]','interpreter','latex')
ylim([1e-1 1e3]); grid on
xlabel('Time Index','interpreter','latex')
...
nexttile % subplot(224)
h3 = pcolor(DATA.T1(Exp_gas,1)',inst_res_PND',[DATA1.PNSD_c(Exp_gas,1),DATA1.PNSD_c(Exp_gas,3:end)]');
set(h3, 'EdgeColor', 'none')
xlabel('Time Index','interpreter','latex')
ylabel('Particle size (nm)','interpreter','latex')
title('PNSD natural gas','interpreter','latex')
colormap jet
caxis([lc, hc])
set(gca, 'YScale', 'log','colorscale','log')
...
%% Only Smoking, PM2.5 [DustTrak] and using PNSD:
...
%% PM2.5 Scatter plots between RI and LCSs

figure(7); fig=gcf;
fig.Position = [100 100 540 400].*2.5;
sgtitle('PM_{2.5}')
subplot(241)
scatter(DATA.SidePak_c(:,1),DATA.DustTrak_c(:,2)); hold on
x = linspace(0,max(DATA.DustTrak_c(:,2)),1e5);
y = linspace(0,max(DATA.DustTrak_c(:,2)),1e5);
plot(x,y,'r');hold off
set(gca, 'XScale', 'log')
set(gca, 'YScale', 'log')
xlabel('SidePak','interpreter','latex')
ylabel('DustTrak','interpreter','latex')
xlim([1e1 1e4]);ylim([1e1 1e4]);grid on

subplot(242)
scatter(DATA.LCS_G1(:,1),DATA.DustTrak_c(:,2));hold on
x = linspace(0,max(DATA.DustTrak_c(:,2)),1e5);
y = linspace(0,max(DATA.DustTrak_c(:,2)),1e5);
plot(x,y,'r');hold off
set(gca, 'XScale', 'log')
set(gca, 'YScale', 'log')
xlabel('$\mathcal{L}_1$','interpreter','latex')
ylabel('DustTrak','interpreter','latex')
xlim([1e-1 1e4]);ylim([1e-1 1e4]);grid on

subplot(243)
scatter(DATA.LCS_G2_01(:,1),DATA.DustTrak_c(:,2));hold on
x = linspace(0,max(DATA.DustTrak_c(:,2)),1e5);
y = linspace(0,max(DATA.DustTrak_c(:,2)),1e5);
plot(x,y,'r');hold off
set(gca, 'XScale', 'log')
set(gca, 'YScale', 'log')
xlabel('$\mathcal{L}_{2a}$','interpreter','latex')
ylabel('DustTrak','interpreter','latex')
xlim([1e-1 1e4]);ylim([1e-1 1e4]);grid on

subplot(244)
scatter(DATA.LCS_G2_02(:,1),DATA.DustTrak_c(:,2));hold on
x = linspace(0,max(DATA.DustTrak_c(:,2)),1e5);
y = linspace(0,max(DATA.DustTrak_c(:,2)),1e5);
plot(x,y,'r');hold off
set(gca, 'XScale', 'log')
set(gca, 'YScale', 'log')
xlabel('$\mathcal{L}_{2b}$','interpreter','latex')
ylabel('DustTrak','interpreter','latex')
xlim([1e-1 1e4]);ylim([1e-1 1e4]);grid on

subplot(245)
scatter(DATA.DustTrak_c(:,2),DATA.SidePak_c(:,1)); hold on
x = linspace(0,max(DATA.DustTrak_c(:,2)),1e5);
y = linspace(0,max(DATA.DustTrak_c(:,2)),1e5);
plot(x,y,'r');hold off
set(gca, 'XScale', 'log')
...
subplot(246)
scatter(DATA.LCS_G1(:,1),DATA.SidePak_c(:,1));hold on
x = linspace(0,max(DATA.SidePak_c(:,1)),1e5);
y = linspace(0,max(DATA.SidePak_c(:,1)),1e5);
plot(x,y,'r');hold off
set(gca, 'XScale', 'log')
set(gca, 'YScale', 'log')
xlabel('$\mathcal{L}_1$','interpreter','latex')
...
subplot(247)
scatter(DATA.LCS_G2_01(:,1),DATA.SidePak_c(:,1));hold on
x = linspace(0,max(DATA.DustTrak_c(:,2)),1e5);
y = linspace(0,max(DATA.DustTrak_c(:,2)),1e5);
plot(x,y,'r');hold off
set(gca, 'XScale', 'log')
...
subplot(248)
scatter(DATA.LCS_G2_02(:,1),DATA.SidePak_c(:,1));hold on
x = linspace(0,max(DATA.DustTrak_c(:,2)),1e5);
y = linspace(0,max(DATA.DustTrak_c(:,2)),1e5);
plot(x,y,'r');hold off
set(gca, 'XScale', 'log')
...
xlabel('PND (CPC)');ylabel('PNSD (CPC)')

figure(9);fig=gcf;
fig.Position = [100 100 540 400].*2.5;
sgtitle('PND (CPC) vs PM$_{2.5}$','interpreter','latex')
...
%% SCATTER PLOTS BETWEEN MET VARS: ACCURACY TEST
